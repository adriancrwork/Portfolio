...
function playSlide(slide, { muted = true, mode = 'hover' } = {}) {
  ...
  iframe.src = `https://www.youtube.com/embed/${id}?autoplay=1&mute=${muteParam}&loop=1&playlist=${id}&controls=0&modestbranding=1&rel=0&playsinline=1&iv_load_policy=3&fs=0&disablekb=1`;
...
function initGallery({ selector, data, captionId }) {
  try {
    ...
    // Forzamos explícitamente el arranque en el primer video real
    // (índice 0 = Video 1 / Horizontal 1), de forma más fiable que
    // solo initialSlide cuando se combina loop + slidesPerView:'auto'.
