...
/* ---------------- Hero: la foto "baja" y se difumina al hacer scroll ----------------
   El wordmark y los textos del hero van en el flujo normal (suben con el scroll
   como cualquier contenido). La foto recibe un desplazamiento y un blur extra
   controlados aquí, para que parezca que "cae" hacia la sección Sobre mí y
   se disuelve, dando paso al contenido. */
...
/* ============================================================
   ...
   Si el CDN no carga, el resto del sitio (hero, nav, scrub,
   reveal) sigue funcionando gracias al try/catch de abajo.
   ============================================================ */
try {
  ...
  dataArray.forEach((item, i) => {
    ...
    slide.innerHTML = `
      ...
      <div class="slide-overlay">
        <p class="slide-index">${String(i + 1).padStart(2, '0')}</p>
...
function setupPlayback(swiperInstance) {
  ...
  // Click / tap (funciona en escritorio y en móvil): reproduce con sonido.
  // Si la tarjeta clickeada no es la central, Swiper la centra primero
  // (slideToClickedSlide) y el siguiente click ya la reproduce.
  swiperInstance.on('click', (swiper) => {
    ...
    if (playing) {
      stopSlide(clicked); // segundo click: pausa
    } else {
      playSlide(clicked, { muted: false, mode: 'click' });
    }
  });

  // Al cambiar de slide, se detiene cualquier reproducción activa
